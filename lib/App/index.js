...
class App {
  ...
  async publish() {
    ...
    if (shouldUpdateVersion.value) {
      ...
      // Make sure no uncommitted changes are left since we are about to commit all staged files in a moment
      if ( await this._git.isGitRepo() ) {
        if( await this._git.hasUncommitedChanges() )
          throw new Error('✖ Please commit your changes to Git first.');
      }
      ...
      if (await fse.exists( this._homeyComposePath )) {
      ...
      if ( await this._git.isGitRepo() ) {
        ...
        if(bumpedVersion) {
          await this._git.commitFiles({
            ...
            description: `Changelog: ${changelogJson[appVersion]['en']}`
          });
          Log(colors.green(`✓ Committed ${commitFiles.map(i => i.replace(`${this.path}/`, '')).join(', and ')} with version bump`));
        } else { // Else commit changelog only
          await this._git.commitFile({
            file: changelogJsonPath,
            message: `Updates changelog for version v${appVersion}`,
            description: `Changelog: ${changelogJson[appVersion]['en']}`
          });
          Log(colors.green(`✓ Committed .homeychangelog.json`));
        }
      }
