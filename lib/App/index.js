...
class App {
  ...
  async _getPackStream() {
    return tmp.file().then( async o => {
      ...
      const fileEntries = await this._getPackFileEntries();
  ...
  async _getPackFileEntries() {
    const fileEntries = await ignoreWalk({
      path: this.path,
    ...
    return fileEntries.filter(entry => {
        ...
        if( entry.charAt(0) === '.' || entry.includes('/.') ) return false;

        // ignore env.json
        if( entry === 'env.json' ) return false;
